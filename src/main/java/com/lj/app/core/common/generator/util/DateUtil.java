...
public class DateUtil {
  ...
  public static String toTimeString(int hour, int minute, int second) {
    ...
    if (hour < MagicNumber.INT_10) {
      hourStr = "0" + hour;
    } else {
      hourStr = "" + hour;
    }
    if (minute < MagicNumber.INT_10) {
      minuteStr = "0" + minute;
    } else {
      minuteStr = "" + minute;
    }
    if (second < MagicNumber.INT_10) {
      secondStr = "0" + second;
    } else {
      secondStr = "" + second;
    }
    if (second == MagicNumber.INT_0) {
  ...
  /**
   * Makes a combined data and time string in the format "MM/DD/YYYY HH:MM:SS" from a Date. If the seconds are 0 they
   * are left off.
   * 
   * @param date
   *          The Date
   * @return A combined data and time string in the format "MM/DD/YYYY HH:MM:SS" where the seconds are left off if they
   *         are 0.
   */
