package com.lj.app.core.common.generator.provider.java.model;
...
import com.lj.app.core.common.generator.provider.java.model.MethodParameter.JavaSourceFileMethodParametersParser;
...
public class JavaMethod {
  ...
  public List<FieldMethodInvocation> getFieldMethodInvocationSequences() {
    if (StringHelper.isNotBlank(clazz.getMavenJavaSourceFileContent())) {
      try {
        JavaMethodInvokeSequencesParser cmd = new JavaMethodInvokeSequencesParser(this,
            clazz.getMavenJavaSourceFileContent());
        cmd.execute();
  ...
  public static class FieldMethodInvocation {
    ...
    public boolean equals(Object obj) {
      if (obj == null)
        return false;
      if (!(obj instanceof FieldMethodInvocation))
        return false;
  ...
  public static class JavaMethodInvokeSequencesParser {
    ...
    private String getMethodBody(String javaSourceContent) {
      String methodStartPattern = "(?s)\\s+" + method.getMethodName() + "\\s*\\("
          + JavaSourceFileMethodParametersParser.getSimpleParamsPattern(method.method) + "\\)\\s*";
      int methodStart = StringHelper.indexOfByRegex(javaSourceContent, methodStartPattern);
      if (methodStart == -1)
        throw new IllegalArgumentException("cannot get method body by pattern:" + methodStartPattern + " methodName:"
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "cannot get method body by pattern:" + methodStartPattern + "\n javaSource:" + javaSourceContent, e);
      }
    }
    ...
    public static String removeJavaComments(String str) {
      ...
      str = str.replaceAll("(?s)/\\*.*?\\*/", ""); // remove block
      // comment: /* */
      return str;
    }
    ...
    // }
    /**
     * 找到对称的一条括号所处的位置, 如 findWrapCharEndLocation("0123{{67}}}",'{','}'), 将返回 [4,9] 如果没有将到,将返回null
     **/
