...
import com.lj.app.core.common.generator.provider.java.model.JavaMethod.FieldMethodInvocation;
...
public class JavaClass {
  ...
  static String getClassName(String simpleJavaType) {
    return simpleJavaType.indexOf('$') >= 0 ? simpleJavaType.substring(simpleJavaType.indexOf('$') + 1)
        : simpleJavaType;
  }
  ...
  public JavaMethod getMethod(String methodName) {
    for (Method m : clazz.getMethods()) {
      if (m.getName().equals(methodName)) {
        return new JavaMethod(m, this); // FIXME
        // javaClazz关联错误,现在会覆盖父类的引用.
      }
    }
    return null;
  }
  ...
  public static class MavenHelper {
    public static String getMavenJavaTestSourceFile(String clazzFile) {
      ...
      clazzFile = clazzFile.replaceAll("\\w+\\$", ""); // for inner class
      // User$1.class
      // and
      // User$Info.class
